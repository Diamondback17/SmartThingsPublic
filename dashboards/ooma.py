...
class OomaAirDialClient:
    ...
    @staticmethod
    def _optional_component(getter, myx_id: str) -> tuple[dict, bool]:
        """Run a component getter, treating 404 (component not present on
        this device) as an empty result rather than a failure. The bool
        says whether the device actually has this component at all (False
        only on 404) - battery/fxs callers ignore it, the lte caller uses
        it as has_lte (200 = this device has LTE hardware, 404 = none)."""
        try:
            return getter(myx_id), True
    ...
    def poll_device(
        ...
        lte_future = self._request_pool.submit(self._optional_component, self.get_lte, myx_id)
        ...
        battery = battery_future.result(timeout=result_timeout)[0] if battery_future else {}
        fxs = fxs_future.result(timeout=result_timeout)[0]
